...
1. **`make check` が緑になるまで完了報告しない。** 落ちたまま「たぶん大丈夫」は禁止。
2. **テストを先に書く。** 振る舞いを検証する。実装の写経になっているテストは価値がない。
3. **スキーマ変更は Flyway のみ。** `ddl-auto` は `validate` 固定。既存マイグレーションは編集せず、常に新しい `V{n}__*.sql` を足す。
4. **秘密情報をコミットしない。** `ANTHROPIC_API_KEY` などは `.env`（gitignore 済み）か GitHub Secrets に置く。
5. **カバレッジ閾値を下げて通さない。** 通らないならテストを足す。
